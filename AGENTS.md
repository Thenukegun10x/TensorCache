...
- **Block-tANS entropy layer (`xs_entropy="tans"`, 0.7+, `tans.py` + `tans_dict.py`):** opt-in alternative to rANS. `PixelCacheWriter(xs_entropy="tans", tans_dict=..., tans_block=256)` codes the same 4 sections (u8/i8 plain, ll4 byte-planar via `pack_byte_planar_tans`) with **block-tANS**: contiguous chunks of B symbols, each an independent stream, plus a K-table **dictionary** learned from a calibration sample (`learn_dictionary_from_arenas`, k-means over block histograms) with a per-block selector. Section format `FMT_VERSION=2`: `[u32 n][u8 fmt][u8 R][u8 K][u32 B][u8 E]` + K tables (only if E=1) + `[u32 nblocks]` + nblocks×u16 `bit_len` + nblocks×u16 `final_state` + selectors (if K>1) + payloads grouped by selector. Dictionary is stored **once** in `_pixel_meta.json["tans"]` as base64 binary (`dictionary_to_meta`/`from_meta`, ~25KB for 4×K=8) and validated on load. **Decode is GPU-only by design** (`build_decode_plan`/`run_decode_plan`, one warp per 32 blocks, one table resident per launch) because CPU tANS decode is ~1.5x *slower* than rANS; `make_xs_loader` therefore requires `num_workers=0` on a CUDA/ROCm device (`require_tans_gpu_decode` raises otherwise). Measured COCO-336: **+5-6% smaller than rANS** (10.56x -> 11.20x @256 imgs, +6.1%), entropy kernels ~50k img/s, e2e `+15%` over wavelet-only at B=128 (~9.4k img/s); the section mix codes *below* the global order-0 Shannon bound because per-block table selection is a conditional model. `cache_images(..., xs_entropy="tans", tans_calib=256)` learns the dictionary in a pre-pass; `tans_dict=` accepts a path/`{kind: tables}`/meta dict to share one. Old caches unaffected (tANS is method id 2; rANS is 1, and v1 has no `enc` keys).
- **Batched GPU encode (0.6+, `codec.py`):** `quantize_pixel_wavelet_adaptive_batched(imgs [N,H,W,3], mode=...)` runs RCT + DWT + per-plane RDO over the batch dim (one torch pass per plane-type via `_quant_adaptive_plane_batched` [Nb,H,W]) and returns N packed_meta dicts **bit-identical** to the per-image encoder (CPU + GPU, 4:2:0/4:4:4). The per-image `quantize_pixel_wavelet_adaptive` is dispatch-bound (~1000+ tiny casts/copies/img -> ~30-45 img/s); batching amortizes launches: measured RX 9070 XT, 336² balanced, N=128 -> ~960 img/s (~28x). `PixelCacheWriter.append_batch(images=<[N,H,W,3] tensor>)` routes through it. Remaining pipeline ceiling is the CPU pack/entropy, not encode.
...
- **rANS needs numba (silent ~50x cliff):** `rans.py` hot loops run under `@njit` when `HAS_NUMBA`, else fall back to pure-Python (`encode`, `decode`) which is ~15-70 ms/img vs ~1 ms/img — enough to make the XS entropy writer the pipeline ceiling even after pack is batched. This now warns **loudly and once per process** from `rans.py`/`tans.py` encode/decode and from `make_xs_loader` (`warn_if_no_numba`), instead of silently crawling. `numba` is in the `[fast]` extra and the lighter `[entropy]` extra (`pip install 'tcache[entropy]'`).
